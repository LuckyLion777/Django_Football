...
env.proj_app = real_project_name("africafa")
...
env.repo_name = conf.get("REPO_NAME", env.proj_app)
env.proj_name = conf.get("PROJECT_NAME", env.proj_app)
env.venv_home = conf.get("VIRTUALENV_HOME", "/home/%s/.virtualenvs" % env.user)
env.venv_path = join(env.venv_home, env.repo_name)
env.proj_path = "/home/%s/mezzanine/%s" % (env.user, env.repo_name)
...
if env.deploy_tool == "git":
    env.repo_path = "/home/%s/git/%s.git" % (env.user, env.repo_name)
else:
...
def install():
    ...
    #sudo("pip install -U pip virtualenv virtualenvwrapper mezzanine mercurial")
    #sudo("pip install -U pip virtualenv virtualenvwrapper mezzanine psycopg2")
    ...
    print(green("Successfully set up git, mercurial, pip, virtualenv, "
                "supervisor, memcached, mezzanine.", bold=True))
...
def create():
    ...
    with cd(env.venv_home):
        if exists(env.repo_name):
            if confirm("Virtualenv already exists in host server: %s"
                       "\nWould you like to replace it?" % env.repo_name):
                run("rm -rf %s" % env.repo_name)
            else:
                abort()
        run("virtualenv %s" % env.repo_name)
    ...
    with project():
        if env.reqs_path:
            pip("-r %s/%s" % (env.proj_path, env.reqs_path))
        pip("gunicorn setproctitle psycopg2 circus "
...
def deploy():
    ...
    via version control or rsync, install new requirements, migrate
    ...
    if not exists(env.proj_path):
        if confirm("Project does not exist in host server: %s"
                   "\nWould you like to create it?" % env.repo_name):
            create()
        else:
            abort()
    ...
    if env.deploy_tool in env.vcs_tools:
        ...
        with cd(join(env.proj_path, "..")):
            ...
            run("tar -cf {0}.tar {1} {0}".format(env.repo_name, exclude_arg))
    ...
    with update_changed_requirements():
        if env.deploy_tool in env.vcs_tools:
            ...
            with cd(env.proj_path.rsplit("/", 1)[0]):
                run("rm -rf %s" % env.repo_name)
                run("tar -xf %s.tar" % env.repo_name)
    with cd(env.proj_path):
        restore("last.db")
    restart()
